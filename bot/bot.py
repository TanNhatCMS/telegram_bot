...
def get_settings_menu(user_id: int):
    ...
    # Chia các nút thành từng hàng với 2 nút mỗi hàng
...
def get_languages_menu(user_id: int):
    current_language = db.get_user_attribute(user_id, "current_language")
    if current_language is None:
        current_language = config.default_language
        db.set_user_attribute(user_id, "current_language", config.default_language)
    text = config.languages["languages"][current_language]["description"]
    text += "\n\n"
    text += "\nSelect <b>languages</b>:"
    # buttons to choose languages
    buttons = []
    for lang_key in config.languages["available_language"]:
        title = config.languages["languages"][lang_key]["name"]
        if lang_key == current_language:
            title = "✅ " + title

        buttons.append(
            InlineKeyboardButton(title, callback_data=f"set_languages|{lang_key}")
        )
...
async def languages_handle(update: Update, context: CallbackContext):
    await register_user_if_not_exists(update, context, update.message.from_user)
    if await is_previous_message_not_answered_yet(update, context): return
    ...
    text, reply_markup = get_languages_menu(user_id)
    await update.message.reply_text(text, reply_markup=reply_markup, parse_mode=ParseMode.HTML)


async def set_languages_handle(update: Update, context: CallbackContext):
    await register_user_if_not_exists(update.callback_query, context, update.callback_query.from_user)
    user_id = update.callback_query.from_user.id
    ...
    _, lang_key = query.data.split("|")
    db.set_user_attribute(user_id, "current_language", lang_key)
    db.start_new_dialog(user_id)
...
async def post_init(application: Application):
    await application.bot.set_my_commands([
        ...
        BotCommand("/languages", "Show setting for languages"),
...
def run_bot() -> None:
    ...
    application.add_handler(CommandHandler("languages", languages_handle, filters=user_filter))
    application.add_handler(CallbackQueryHandler(set_languages_handle, pattern="^set_languages"))
